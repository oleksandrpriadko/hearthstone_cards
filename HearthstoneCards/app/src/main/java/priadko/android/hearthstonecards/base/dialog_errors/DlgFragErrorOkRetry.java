...
import priadko.android.hearthstonecards.base.base_fragment.DlgFragAbstractOkCancel;
 ...
 * Oleksandr Priadko
 */
public class DlgFragErrorOkRetry extends DlgFragAbstractOkCancel {
    ...
    protected void performCancel(DialogInterface dialog, int id) {
        if (targetActivity != null) {
            targetActivity.onCustomActivityResult(
                    null,
                    requestCode,
                    ActivityBaseCustomResult.RESULT_CANCELED);
        } else if (getTargetFragment() != null) {
            getTargetFragment().onActivityResult(getTargetRequestCode(),
                    ActivityBaseCustomResult.RESULT_CANCELED,
                    null);
